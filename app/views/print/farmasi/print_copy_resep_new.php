...
$pdf->Cell(0, 4, $data['hdr']['data']['Iter'].'x', 0, 1);
...
foreach ($data['listdata'] as $row) {
    ...
    if ($row['Racik'] == '0'){
            ...
            $pdf->Cell(6, 4, '', 0, 0);
            $pdf->Cell(0, 4, 'S. ' . $row['Signa'] . ' ' . $row['Keterangan1'] . ' ' . $row['Keterangan2'], 0, 0);
    
            //var_dump($row['Qty']);
            if ($row['QryRealisasi'] == $row['QryOrder']) {
                $pdf->Cell(0, 4, 'Detur', 0, 1, 'R');
            } elseif ($row['QryRealisasi'] <> $row['QryOrder']) {
                if ($row['QryRealisasi'] == 0) {
                    $pdf->Cell(0, 4, 'Nedet.', 0, 1, 'R');
                } elseif ($row['QryRealisasi'] > 0) {
                    $pdf->Cell(0, 4, 'Det : ' . $row['QryRealisasi'], 0, 1, 'R');
                }
            }
            //BR
            $pdf->Cell(0, 1, '', 0, 1);
        ...
        if ($row['Racik'] <> 0){
            ...
            //     $pdf->Cell(6, 4, '', 0, 0);
            //     $pdf->Cell(0, 4, 'S. ' . $row['Signa'] . ' ' . $row['Keterangan1'] . ' ' . $row['Keterangan2'], 0, 1);
            //     $pdf->Cell(0, 3, '', 0, 1);
            // }
            ...
            if ($row['Header'] == '1' ){
                ...
                $pdf->Cell(0, 4, $row['NamaBarang'], 0, 1);
                // $pdf->SetFont('Arial', '', 9);
                // $pdf->Cell(6, 4, '', 0, 0);
                // $pdf->Cell(0, 4, $row['NamaBarang'] . ' ( Dosis: ' . $row['Dosis'] . ' ' . $row['KetDosis'] . ' ; Qty: ' . $row['QryRealisasi'] . ')', 0, 1);
                // $pdf->Cell(0, 1, '', 0, 1);
                // $qtyracikanheader = $row['QryRealisasi'];
                //continue;
                $signaracik = $row['Signa'];
                $ket1racik = $row['Keterangan1'];
                $ket2racik = $row['Keterangan2'];
                //continue;
            }
