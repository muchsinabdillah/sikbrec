...
foreach ($data['listdata'] as $row) {
    ...
    if ($row['Racik'] == '0'){
        ...
        $pdf->Cell(6, 4, 'R!', 0, 0);
        $pdf->SetFont('Arial', '', 9);
        //NO : '+CONVERT(varchar(15), a.QryOrder)+' (QtyReal : '+CONVERT(varchar(15), a.QryRealisasi)+' )'
        $pdf->Cell(0, 4, $row['NamaBarang'].' NO : '.$row['QryOrder'].' (QtyReal : '.$row['QryRealisasi'].' )', 0, 1);
        $pdf->Cell(6, 4, '', 0, 0);
        $pdf->Cell(0, 4, 'S. ' . $row['SignaTerjemahan'] . ' ' . $row['Keterangan1'] . ' ' . $row['Keterangan1'], 0, 1);
        //BR
        $pdf->Cell(0, 1, '', 0, 1);
        continue;
    }
