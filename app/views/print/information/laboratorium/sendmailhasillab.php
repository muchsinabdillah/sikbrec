...
/*
$qr = "SELECT [E-Mail Address] as email FROM MasterDataSQL.dbo.Admision a
...
*/
       ...
       //$emailcc = "abdullahfiqri@gmail.com";
require '../App/library/classes/class.phpmailer.php'; 
$mail = new PHPMailer;
    $mail->isSMTP();                                // Set mailer to use SMTP
    $mail->Host = "srv76.niagahoster.com";                       // SMTP server
    $mail->SMTPAuth = true;                         // Enable SMTP authentication
    $mail->Username = "itsupport@rsyarsi.co.id";                 // SMTP username
    $mail->Password = "#x!dev.@rsyarsi3000$";                 // SMTP password
    $mail->SMTPSecure = 'ssl';                      // Enable TLS encryption, `ssl` also accepted
    $mail->From = "itsupport@rsyarsi.co.id";
    $mail->Port = 465;   
    $mail->SMTPDebug = 2;                           // SMTP Port
    $mail->FromName  = 'LABORATORY RS YARSI';
    $mail->IsHTML(true);
...
Ditempat</p>
<p style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont,  Roboto, Helvetica, Arial, sans-serif, ; position: relative; font-size: 16px; line-height: 1.5em; margin-top: 0; text-align: left;">Berikut kami lampirkan hasil laboratorium pemeriksaan yang telah dilakukan di RS yarsi.</p>
...
<p style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont,  Roboto, Helvetica, Arial, sans-serif, ; position: relative; font-size: 16px; line-height: 1.5em; margin-top: 0; text-align: left;">Instalasi laboratorium RS yarsi</p>
</body>
</html>
    ';
    $mail->AddAddress($data['email']['email']);
    $mail->addCC($emailcc);
    //$mail->addStringAttachment($pdf->Output("S",'Hasillab-'.$nolab.'.pdf'), 'Hasillab-'.$nolab.'.pdf', $encoding = 'base64', $type = 'application/pdf');
    $mail->addStringAttachment(file_get_contents($data['listdata1']['AwsUrlDocuments']), 'Hasillab-'.$nolab.'.pdf', $encoding = 'base64', $type = 'application/pdf');

if(!$mail->send()) {
    $arrResult = array ('status'=>'error', 'message' => 'Gagal Mengirim, errorinfo: '. $mail->ErrorInfo);
} else {
    $arrResult = array ('status'=>'success', 'message' => 'Berhasil Mengirim E-Mail Ke '. $data['email']['email']);
}
//return $arrResult;
