...
class PDF extends Tcpdf
{
    function Header()
    {
        ...
        $this->Ln(5);
        $this->Cell(206, 4, '', 0, 0);
        $this->Cell(69, 4, "                                  " . $GLOBALS['headercetakanx']['AlamatRS'], 0, 1);
        $this->Cell(220, 4, '', 0, 0);
...
$x = $pdf->GetX();
$y = $pdf->GetY();
$pdf->MultiCell(10, $hb, 'NO.', $border, 1);
$pdf->SetXY($x + 10, $y);
...
$pdf->SetXY($x + 165, $y);
$pdf->MultiCell(20, $hb, 'NOMINAL BAYAR', $border, 1);
$pdf->SetXY($x + 185, $y);
...
for ($i = 0; $i < $countDetail; $i++) {
    ...
    // $x = $pdf->GetX();
    // $y = $pdf->GetY();
    $pdf->CellFitScale(10, $hb, $no, $border, 0);
    $pdf->CellFitScale(25, $hb, $GLOBALS['detail'][$i]['TGL_Transaksi'] . "\n" . $GLOBALS['detail'][$i]['NoTransaksi'], $border, 0);
    // $pdf->SetXY($x + 25, $y);
    $pdf->CellFitScale(25, $hb, $GLOBALS['detail'][$i]['NoReg'], $border, 0);
    // $pdf->SetXY($x + 50, $y);
    $pdf->CellFitScale(35, $hb,  $GLOBALS['detail'][$i]['NamaPasien'], $border, 0);
    // $pdf->SetXY($x + 85, $y);
    $pdf->CellFitScale(35, $hb, $GLOBALS['detail'][$i]['Nama_Unit'] . "\n" . $GLOBALS['detail'][$i]['NamaJaminan'], $border, 0);
    // $pdf->SetXY($x + 120, $y);
    $pdf->CellFitScale(35, $hb, $GLOBALS['detail'][$i]['Nama_Dokter'], $border, 0);
    // $pdf->SetXY($x + 155, $y);
    $pdf->CellFitScale(20, $hb, number_format($GLOBALS['detail'][$i]['Nominal_Bayar']), $border, 0);
    // $pdf->SetXY($x + 180, $y);
    $pdf->CellFitScale(21, $hb, number_format($GLOBALS['detail'][$i]['Cash']), $border, 0);
    // $pdf->SetXY($x + 201, $y);
    $pdf->CellFitScale(21, $hb, number_format($GLOBALS['detail'][$i]['Debit']), $border, 0);
    // $pdf->SetXY($x + 222, $y);
    $pdf->CellFitScale(21, $hb, number_format($GLOBALS['detail'][$i]['Kredit']), $border, 0);
    // $pdf->SetXY($x + 243, $y);
    $pdf->CellFitScale(21, $hb, number_format($GLOBALS['detail'][$i]['Piutang']), $border, 0);
    // $pdf->SetXY($x + 264, $y);
    $pdf->CellFitScale(21, $hb, number_format($GLOBALS['detail'][$i]['Qris']), $border, 1);
}
$pdf->setFont('', 'B', 7);
$pdf->Cell(165, $hb, 'GRAND TOTAL', $border, 0, 'C');
$pdf->Cell(20, $hb, number_format($totalnominalbayar), $border, 0);
