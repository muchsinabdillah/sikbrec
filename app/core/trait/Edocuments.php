...
trait Edocuments
{
    function PrintFile ($data){
        $filedoc = '../app/views/'.$data['filedoc'].'.php';
        include($filedoc);
        $fileName = $data['jeniscetakan'].'-'.$data['notrs'].'.pdf';
...
try {
    ...
    $email_to = $data['email_send'];
    $mail->addAddress($email_to);     //Add a recipient
 
    //Content
    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject   = $data['judul'];
    ...
    Ditempat</p>
    <p style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont,  Roboto, Helvetica, Arial, sans-serif, ; position: relative; font-size: 16px; line-height: 1.5em; margin-top: 0; text-align: left;">Berikut kami lampirkan file '.$data['judul'].'.</p>
    ...
    <p style="box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont,  Roboto, Helvetica, Arial, sans-serif, ; position: relative; font-size: 16px; line-height: 1.5em; margin-top: 0; text-align: left;">RS yarsi</p>
        ...
        ';
    $mail->AltBody = '';
    //mail->AddEmbeddedImage('../public/images/yarsi.png', 'logo'); //abaikan jika tidak ada logo
    //$mail->addAttachment(''); 
    $mail->addStringAttachment(file_get_contents($data['aws_url']), $data['judul'].'-'.$data['notrs'].'.pdf',  $encoding = 'base64', $type = 'application/pdf');
