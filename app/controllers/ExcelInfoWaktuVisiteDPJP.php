<?php
class ExcelInfoWaktuVisiteDPJP extends Controller
{
    public function ExcelInfoWaktuVisiteDPJP2($TglAwal = "", $TglAkhir = "")
    {
        ...
        $JobOrder =   $this->model('B_InformationWaktuTunggu_Model')->getDataListPasien_Ranap1($data);
        $dataJO = json_decode(json_encode((object) $JobOrder), FALSE);
        $dataJOx = json_encode((object) $JobOrder);


        $excel->setActiveSheetIndex(0)->setCellValue('A1', "Info Waktu Visite DPJP"); // Set kolom A1 dengan tulisan "DATA SISWA"
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('D5', "Dokter"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"

        $excel->setActiveSheetIndex(0)->setCellValue('E5', "Tgl Masuk"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"

        $excel->setActiveSheetIndex(0)->setCellValue('F5', "Jam Masuk"); // Set kolom E3 dengan tulisan "ALAMAT"

        $excel->setActiveSheetIndex(0)->setCellValue('G5', "Tgl Cppt"); // Set kolom E3 dengan tulisan "ALAMAT"

        $excel->setActiveSheetIndex(0)->setCellValue('H5', "Jam Cppt"); // Set kolom E3 dengan tulisan "ALAMAT"
        ...
        foreach ($JobOrder as $data) { // Lakukan looping pada variabel siswa
            ...
            $excel->setActiveSheetIndex(0)->setCellValue('D' . $numrow, $data['NamaUser']);
            $excel->setActiveSheetIndex(0)->setCellValue('E' . $numrow, $data['TglMasuk']);
            $excel->setActiveSheetIndex(0)->setCellValue('F' . $numrow, $data['JamMasuk']);
            $excel->setActiveSheetIndex(0)->setCellValue('G' . $numrow, $data['TglCPPT']);
            $excel->setActiveSheetIndex(0)->setCellValue('H' . $numrow, $data['JamCPPT']);
        ...
        $excel->getActiveSheet(0)->setTitle("Info Waktu Visite DPJP");
        $excel->setActiveSheetIndex(0);

        // Proses file excel
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Info Waktu Visite DPJP.xls"'); // Set nama file excel nya
