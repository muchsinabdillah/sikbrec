<?php
class ExcelInfoPelunasanHutangDetail extends Controller
{
    public function ExcelInfoPelunasanHutangDetail2($TglAwal = "", $TglAkhir = "")
    {
        ...
        $JobOrder =   $this->model('I_Pelunasan_Hutang_Model')->getInfoPelunasanHutangDetail1($data);
        $dataJO = json_decode(json_encode((object) $JobOrder), FALSE);
        $dataJOx = json_encode((object) $JobOrder);


        $excel->setActiveSheetIndex(0)->setCellValue('A1', "Informasi Pelunasan Hutang Detail"); // Set kolom A1 dengan tulisan "DATA SISWA"
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('F5', "Company"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"

        $excel->setActiveSheetIndex(0)->setCellValue('G5', "Nilai Pay"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"

        $excel->setActiveSheetIndex(0)->setCellValue('H5', "Sisa Hutang"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"

        $excel->setActiveSheetIndex(0)->setCellValue('I5', "Periode Hutang Awal"); // Set kolom D3 dengan tulisan "JENIS KELAMIN"

        $excel->setActiveSheetIndex(0)->setCellValue('J5', "Periode Hutang Akhir"); // Set kolom E3 dengan tulisan "ALAMAT"

        $excel->setActiveSheetIndex(0)->setCellValue('K5', "Rekening Pelunasan"); // Set kolom E3 dengan tulisan "ALAMAT"

        $excel->setActiveSheetIndex(0)->setCellValue('L5', "Nama Rekening"); // Set kolom E3 dengan tulisan "ALAMAT"
        ...
        foreach ($JobOrder as $data) { // Lakukan looping pada variabel siswa
            ...
            $excel->setActiveSheetIndex(0)->setCellValue('F' . $numrow, $data['Company']);
            $excel->setActiveSheetIndex(0)->setCellValue('G' . $numrow, $data['NILAI_PAY']);
            $excel->setActiveSheetIndex(0)->setCellValue('H' . $numrow, $data['SISA_HUTANG']);
            $excel->setActiveSheetIndex(0)->setCellValue('I' . $numrow, $data['PeriodeHutangAwal']);
            $excel->setActiveSheetIndex(0)->setCellValue('J' . $numrow, $data['PeriodeHutangAkhir']);
            $excel->setActiveSheetIndex(0)->setCellValue('K' . $numrow, $data['REK_PELUNASAN']);
            $excel->setActiveSheetIndex(0)->setCellValue('L' . $numrow, $data['FS_NM_REKENING']);
        ...
        $excel->getActiveSheet(0)->setTitle("Info Pelunasan Hutang Detail");
        $excel->setActiveSheetIndex(0);

        // Proses file excel
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Informasi Pelunasan Hutang Detail.xlsx"'); // Set nama file excel nya
