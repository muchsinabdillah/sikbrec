...
async function goshowGrafik() {
    try
    {
        await showGrafikRekapTahun(); 
        await showGrafikBulanan();  
    } catch (err) {
        toast(err, "error")
        console.log(err)
    }
}
function showGrafikRekapTahun() { 
    var Info_Date_Start = document.getElementById("Info_Date_Start").value; 
    const base_url = window.location.origin;
    $.ajax({
        "url": base_url + '/SIKBREC/public/GrafikKunjunganPasien/RekapTahun/',
        type: "POST",
...
function showGrafikBulanan() { 
    ...
    const base_url = window.location.origin;
    $.ajax({
        "url": base_url + '/SIKBREC/public/GrafikKunjunganPasien/Bulanan/',
        type: "POST",
        data:  "&Info_Date_Start=" + Info_Date_Start,
        cache: false,
        dataType: "JSON",
