...
function showDataPasienRajalAktif() {
    ...
    $('#tbl_aktif').DataTable({
        ...
        //"order": [[ 2, "desc" ]],
        "ajax": {
            "url": base_url + "/SIKBREC/public/aRegistrasiRajal/showDataPasienRajalAktifMCU",
            "dataSrc": "",
            "deferRender": true,
            "type": "POST",
            data: function (d) {
            ...
            { "data": "namauser" }, 
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    ...
                    if (row.status_kehadiran != null){
                        var html = '<font size="2"><span class="label label-info">SUDAH SCAN BARCODE</span></font>';
                    }else{
                        var html = '<font size="2"><span class="label label-warning">BELUM SCAN BARCODE</span></font>';
                    }
                    return html
                }
            },
...
function goScan() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/aRegistrasiRajal/scanQrCodeKehadiran";
}
...
function ApproveAll(noted) {
    ...
    var base_url = window.location.origin;
    let url = base_url + '/SIKBREC/public/aRegistrasiRajal/sendreminderAllMCU';
    return fetch(url, {
        method: 'POST',
        headers: {
            "Content-type": "application/x-www-form-urlencoded; charset=UTF-8"
