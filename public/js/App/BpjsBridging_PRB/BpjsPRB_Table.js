...
function showDataPasienRajalArsip(){
    ...
    PeriodeAwal = $("#tglAwalarsip").val();
    PeriodeAkhir = $("#tglAkhirArsip").val(); 
    $(".preloader").fadeOut();
    $('#example').dataTable({
        "bDestroy": true
    }).fnDestroy();
    $('#example').DataTable({
        "ordering": true,
        "ajax": {
            "url": base_url + "/SIKBREC/public/xBPJSBridging_PRB/getDataList",
            "dataSrc": "",
            "deferRender": true,
            "type": "POST",
            data: function (d) {
        ...
        "columns": [
        {
            ...
            },
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.LokasiPasien + ' </font>  ';
                    return html
                }
            },
            {
            ...
            },
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.PRB + ' </font>  ';
                    return html
                }
            },
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.INPUT_PRB + ' </font>  ';
                    return html
                }
            },
            {
...
function CreateNew() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/xBPJSBridging_PRB/viewData/";
}