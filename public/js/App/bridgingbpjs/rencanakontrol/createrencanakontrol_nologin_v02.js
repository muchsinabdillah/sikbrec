...
function updateUICreateRencanaKontrol(params) {
    ...
    swal("Success", "Rencana Kontrol Berhasil Dibuat ! ", "success")
        .then((value) => {
            ...
            url = "http://172.16.2.2:8080/SIRYARSI/Halaman/Pendaftaran/Reservasi_Emr.php?nomr="+$('#SIMRS_NoMR').val()+"&note=&noregistrasi="+$("#SIMRS_Registrasi").val() +"&noepsiode="+$("#SIMRS_NoEpisode").val();
...
function getDataSPRDetail() {
    var base_url = window.location.origin; 
    var id = $("#SIMRS_Registrasi").val();
    let url = base_url + '/SIKBREC/public/aKontrolUlang/getDataPasien/';
    return fetch(url, {
        method: 'POST',
        headers: {
            "Content-type": "application/x-www-form-urlencoded; charset=UTF-8"
