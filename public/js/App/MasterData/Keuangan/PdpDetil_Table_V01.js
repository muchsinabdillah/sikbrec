$(document).ready(function () {
    ...
    $('#example').DataTable({
        "ordering": true,
        "ajax": {
            "url": base_url + "/SIKBREC/public/PdpDetil/getAllPdpDetil",
            "dataSrc": "",
            "deferRender": true,
        },
        "columns": [
            ...
            },
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.KD_TIPE_PDP + ' </font>  ';
                    return html
                }
            },
            {
...
function showDataGroupShift(str) {
    const base_url = window.location.origin;
    var str = btoa(str);
    window.location = base_url + '/SIKBREC/public/PdpDetil/viewPdpDetil/' + str;
}

function goGroupShiftPages() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/PdpDetil/viewPdpDetil";
}