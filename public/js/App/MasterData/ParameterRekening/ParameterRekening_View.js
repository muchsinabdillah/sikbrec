...
async function saveMasterUnit() {
    ...
    let url = base_url + '/SIKBREC/public/ParameterRekening/saveParameterRekening';
...
function updateUIgetDataRefferal(datagetDataUnit) {
    ...
    $("#parameter").val(convertEntities(dataResponse.data.parameter));
    $("#rekening").val(convertEntities(dataResponse.data.rekening));
}
function getDataUnit() {
    var base_url = window.location.origin;
    let url = base_url + '/SIKBREC/public/ParameterRekening/getDatabyId/';
    return fetch(url, {
        method: 'POST',
        headers: {
            "Content-type": "application/x-www-form-urlencoded; charset=UTF-8"
...
function MyBack() {
    const base_url = window.location.origin;
     window.location = base_url + "/SIKBREC/public/ParameterRekening/list";
}
