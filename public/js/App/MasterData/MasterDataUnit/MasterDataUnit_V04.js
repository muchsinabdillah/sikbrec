$(document).ready(function () { 
    ...
    saveButton.addEventListener('click', async function () {
        try {
            const result = await saveMasterUnit();
...
async function saveMasterUnit() {
    ...
    let url = base_url + '/SIKBREC/public/MasterDataUnit/addUnit';
...
async function asyncShowMain() {
    try { 
        ...
        const datagetDataUnit = await getDataUnit();
        console.log(datagetDataUnit)
    
        updateUIgetDataRefferal(datagetDataUnit);
    } catch (err) {
       
        toast(err, "error")
    }
}
function updateUIgetDataRefferal(datagetDataUnit) {
    let dataResponse = datagetDataUnit;
    $("#IdAuto").val(convertEntities(dataResponse.data.ID));
    $("#CodeUnit").val(convertEntities(dataResponse.data.CODEUNIT));
    $("#NamaUnit").val(convertEntities(dataResponse.data.NamaUnit));
    $("#EMRMenu").val(convertEntities(dataResponse.data.EMRMenu)).trigger('change');
    $("#CodeRegis").val(convertEntities(dataResponse.data.CodeRegis)).trigger('change');
    $("#CodeAntrian").val(convertEntities(dataResponse.data.CodeAntrian));
    $("#GrupInstalasi").val(convertEntities(dataResponse.data.grup_instalasi)).trigger('change');
    $("#ActiveData").val(convertEntities(dataResponse.data.active)).trigger('change');
    $("#CodeBPJS").val(convertEntities(dataResponse.data.codeBPJS));
}
...
function MyBack() {
    const base_url = window.location.origin;
     window.location = base_url + "/SIKBREC/public/MasterDataUnit/";
}
