...
function showdatatabel() {
    ...
    $('#example').DataTable({
        "ordering": true,
        "ajax": {
            "url": base_url + "/SIKBREC/public/MasterLiburNasional/getAllData",
            "dataSrc": "",
            "deferRender": true,
        },
        "columns": [
...
function showDataGroupShift(str) {
    const base_url = window.location.origin;
    var str = btoa(str);
    window.location = base_url + '/SIKBREC/public/MasterLiburNasional/views/' + str;
}

function goGroupShiftPages() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/MasterLiburNasional/views/";
}