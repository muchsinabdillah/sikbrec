...
function loadtabel() {
    ...
    $('#example').DataTable({
        "ordering": true,
        "ajax": {
            "url": base_url + "/SIKBREC/public/TarifKamar/getAllTarifKamar",
            "dataSrc": "",
            "deferRender": true,
        },
        "columns": [
            ...
            },
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.Group_Tarif + ' </font>  ';
                    return html
                }
            },
            {
            ...
            },
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.Tanggal_Berlaku + ' </font>  ';
                    return html
                }
            },
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.Tanggal_Expired + ' </font>  ';
                    return html
                }
            },
            {
...
function loadtabelbyLantai() {
    ...
    $('#example').DataTable({
        "ordering": true,
        "ajax": {
            "url": base_url + "/SIKBREC/public/TarifKamar/getAllTarifKamarByGroupTarif",
            "dataSrc": "",
            "deferRender": true,
            "type": "POST",
            data: function (d) {
...
function showDataGroupShift(str) {
    const base_url = window.location.origin;
    var str = btoa(str);
    window.location = base_url + '/SIKBREC/public/TarifKamar/viewTarifKamar/' + str;
}

function goGroupShiftPages() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/TarifKamar/viewTarifKamar/";
}
