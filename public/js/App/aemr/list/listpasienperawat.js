        ...
        "columns": [  
            { 
                "render": function (data, type, row) {
                    ...
                    <li><a href="javascript:void(0)" onclick="Icare('${row.ID}')">I-Care</a></li>
                    <li><a href="javascript:void(0)" onclick="ShowHistoryDocuments('${row.ID}')" >E-Document</a></li>
            ...
            { "data": "DokterName" }, 
            {
                "render": function (data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    var html = '<font size="1"> ' + row.Perusahaan + '</br> <b>' + row.NoSEP_edited + ' </b></font>';
            ...
            { "data": "namauser" }, 
            {
                "render": function (data, type, row) {
                    var html = "" 
                    var html = `
                        <button type="button" class="btn btn-default border-primary btn-animated btn-xs" onclick="ShowDataPasienPoliklinik('${row.NoRegistrasi}')" ><span class="visible-content" > EMR</span></button> 
                        &nbsp
                        <button type="button" class="btn btn-default border-primary btn-animated btn-xs" onclick="ShowHistoryDocuments('${row.NoRegistrasi}')" ><span class="visible-content" > Panggil Pasien</span></button>
                            `;
                    return html
                } 
            },
...
function ShowDataPasienPoliklinik(str) {
    ...
    window.location = base_url + '/SIKBREC/public/aEMR/AssesmentRawatJalan/' + str;
}
function gocreate() {
    const base_url = window.location.origin;
