...
$(document).ready(function () {  
    ...
    $('#formUploadManual').submit(
        function (e) {
            ...
            form_data.append("jeniskeluarga", $("#jeniskeluarga").val()); 
            form_data.append("tptkerja", $("#tptkerja").val()); 
            form_data.append("tgllahir", $("#tgllahir").val()); 
            form_data.append("namakeluarga", $("#namakeluarga").val()); 
            form_data.append("jenisdoc", $("#jenisdoc").val()); 
            $.ajax({
                url: base_url + '/SIKBREC/public/FormDataPegawai/uploadDataKeluarga/',
                type: 'POST',
                data: form_data,
                processData: false,
                contentType: false,
...
function updateUGetDataPegawaiById2(data){
    ...
    $("#tptkerja").val(data.Tpt_Lahir); 
    $("#tgllahir").val(data.Tgl_Lahir); 
    $("#namakeluarga").val(data.NamaKeluarga);
    $("#jeniskeluarga").val(data.StatusKeluarga);
...
function GetDataPegawaiById2() {
    ...
    let url = base_url + '/SIKBREC/public/FormDataPegawai/getDataKeluargaDetail';
    return fetch(url, {
        method: 'POST',
        headers: {
            "Content-type": "application/x-www-form-urlencoded; charset=UTF-8"
...
function LoadData() {
    ...
    $('#example').DataTable({
        ...
        //"order": [[ 2, "desc" ]],s
        "ajax": {
            "url": base_url + '/SIKBREC/public/FormDataPegawai/getDataListKeluarga',
            "dataSrc": "",
            "deferRender": true,
            "type": "POST",
            data: function (d) {
...
function showDataGroupShift(str) {
    const base_url = window.location.origin;
    window.location = base_url + '/SIKBREC/public/FormDataPegawai/UpdateDataKeluarga/' + str;
