...
$(document).ready(function () {  
    ...
    $('#formUploadManual').submit(
        function (e) {
            ...
            form_data.append("jenisPendidikan", $("#jenisPendidikan").val()); 
            form_data.append("tptInst", $("#tptInst").val()); 
            form_data.append("tglLulus", $("#tglLulus").val()); 
            form_data.append("jurusan", $("#jurusan").val()); 
            // form_data.append("namakeluarga", $("#namakeluarga").val()); 
            form_data.append("jenisdoc", $("#jenisdoc").val()); 
            $.ajax({
                url: base_url + '/SIKBREC/public/FormDataPegawai/uploadDataPendidikan/',
                type: 'POST',
                data: form_data,
                processData: false,
                contentType: false,
...
function updateUGetDataPegawaiById2(data){
    ...
    $("#NamaPegawai").val(data.Nama);
    $("#nikPegawai").val(data.Nip); 
    $("#jenisPendidikan").val(data.Jenis_Pendidikan);
    $("#tptInst").val(data.Nama_Pendidikan);
    $("#tglLulus").val(data.Tahun_Lulus); 
...
function GetDataPegawaiById2() {
    ...
    let url = base_url + '/SIKBREC/public/FormDataPegawai/getDataPendidikanDetail';
    return fetch(url, {
        method: 'POST',
        headers: {
            "Content-type": "application/x-www-form-urlencoded; charset=UTF-8"
...
function LoadData() {
    ...
    $('#example').DataTable({
        ...
        //"order": [[ 2, "desc" ]],s
        "ajax": {
            "url": base_url + '/SIKBREC/public/FormDataPegawai/getDataListPendidikan',
            "dataSrc": "",
            "deferRender": true,
            "type": "POST",
            data: function (d) {
...
function showDataGroupShift(str) {
    const base_url = window.location.origin;
    window.location = base_url + '/SIKBREC/public/FormDataPegawai/UpdateDataPendidikan/' + str;
