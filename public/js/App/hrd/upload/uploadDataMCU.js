...
$(document).ready(function () {  
    ...
    $('#formUploadManual').submit(
        function (e) {
            ...
            form_data.append("tglMCU", $("#tglMCU").val()); 
            form_data.append("tglHasil", $("#tglHasil").val()); 
            form_data.append("Keterangan", $("#Keterangan").val());

            // form_data.append("jenisdoc", $("#jenisdoc").val()); 
            $.ajax({
                url: base_url + '/SIKBREC/public/FormDataPegawai/uploadDataMCU/',
                type: 'POST',
                data: form_data,
                processData: false,
                contentType: false,
...
function updateUGetDataPegawaiById2(data){
    ...
    $("#tglMCU").val(data.tglMCU); 
    $("#tglHasil").val(data.tglHasil);
...
function GetDataPegawaiById2() {
    ...
    let url = base_url + '/SIKBREC/public/FormDataPegawai/getDataMCUDetail';
    return fetch(url, {
        method: 'POST',
        headers: {
            "Content-type": "application/x-www-form-urlencoded; charset=UTF-8"
...
function LoadData() {
    ...
    $('#example').DataTable({
        ...
        //"order": [[ 2, "desc" ]],s
        "ajax": {
            "url": base_url + '/SIKBREC/public/FormDataPegawai/getDataListMCU',
            "dataSrc": "",
            "deferRender": true,
            "type": "POST",
            data: function (d) {
...
function showDataGroupShift(str) {
    const base_url = window.location.origin;
    window.location = base_url + '/SIKBREC/public/FormDataPegawai/UpdateDataMCU/' + str;
