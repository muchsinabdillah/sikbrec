...
function synctransaction() {
    ...
    return fetch(url, {
        ...
        body: 'owlexa_cardnumber=' + owlexa_cardnumber
        + "&owlexa_claimnumber=" + owlexa_claimnumber
...
function updateShowSyncTransaction(datagetShowSyncTransaction) {
    $("#NoTagihan").val(datagetShowSyncTransaction.FS_KD_TRS);
    $("#tgltagih").val(datagetShowSyncTransaction.FD_TGL_TRS);
    $("#JenisPenjamin").val(datagetShowSyncTransaction.FS_JENIS_CUSTOMER).trigger('change');
    getIDPenjamin();
    setTimeout(function() {
        $("#kodejaminan").val(datagetShowSyncTransaction.FS_KD_CUSTOMER).trigger('change');
    }, 100);
    // console.log('idasuransi', datagetShowSyncTransaction.FS_KD_CUSTOMER);
    //$('#kodejaminan').val(datagetShowSyncTransaction.FS_KD_CUSTOMER).trigger('change'); 
    $("#kodejaminantemp").val(datagetShowSyncTransaction.FS_KD_CUSTOMER);
    $("#NoSuratTagihan").val(datagetShowSyncTransaction.FS_KET);
    $("#NominalTagihan").val(datagetShowSyncTransaction.FN_TOTAL_TAGIH);
    $("#Periode2").val(datagetShowSyncTransaction.fd_periode2);
    $("#Periode").val(datagetShowSyncTransaction.fd_periode1);
    $("#Fs_Code_Jenis_Reg").val(datagetShowSyncTransaction.Fs_Code_Jenis_Reg);

    $("#alamattagih").val(datagetShowSyncTransaction.FS_Alamat_Tagih);
...
function bataltagih(row_id) {
    ...
    return fetch(url, {
        ...
        body:'NoTrs=' + NoTrs
        +'&tgltagih=' + tgltagih + '&Periode=' + Periode
        + '&Periode2=' + Periode2
...
async function tagih(row_id) {
    try {
        const datatagih = await tagih(row_id); 
...
function tagih(row_id) {
    ...
    return fetch(url, {
        ...
        + '&kodejaminan=' + kodejaminan
        + '&NoTagihan=' + NoTagihan
        + '&Fs_Code_Jenis_Reg=' + Fs_Code_Jenis_Reg
...
function getNamaPenjamin() {
    ...
    return fetch(url, {
        ...
        body: 'JenisPenjamin=' + $("#JenisPenjamin").val()
...
function updateOrderPiutangbyId(datagetOrderPiutangbyId) {
    ...
    $('#table-id2').DataTable({
        ...
        "columns": [{
            ...
            },
            {
                "render": function(data, type, row) { // Tampilkan kolom aksi
                    var html = ""
                    if (row.FB_TAGIH == "1") { // Jika jenis kelaminnya 1
                        var html =
             ...
             "ajax": {
              ...
              "columns": [  
                { "render": function ( data, type, row ) { // Tampilkan kolom aksi 
                  ...
                  { "data": "FN_TOTAL_TAGIH" ,  render: $.fn.dataTable.render.number( '.', ',', 0,'' )},  // Tampilkan nama   
...
function MyBack() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/Piutang/listOrderPembayaranPiutang";
}
