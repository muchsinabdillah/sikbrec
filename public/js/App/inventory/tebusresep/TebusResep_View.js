...
function updateUIdatacreateHeader(params) {
    ...
    if (params.status === true) {
        ...
        $("#No_Transaksi").val(params.data.TransactionCode);
        $("#NoRegistrasi").val(params.data.NoRegistrasi);
        unlockBtnCreate();
...
function createHeader() {
    ...
    var url2 = "/SIKBREC/public/aTebusResep/addTebusResep";
    var base_url = window.location.origin;
...
function updateUIdataHeader(params) {
    if (params.status === true) {
        ...
        $("#IdDokter").val(params.data[0].IdDokter);
        ...
        $("#HasilReview").val(params.data[0].HasilReview);
...
function goVoidSales(param) {
    ...
    var url2 = "/SIKBREC/public/aTebusResep/voidSalesTebus";
    var base_url = window.location.origin;
    let url = base_url + url2;
    return fetch(url, {
        method: 'POST',
...
function MyBack() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/aTebusResep/list";
    //location.reload();
}
...
function updateUIdataunitbyIP(dataResponse) {
    if (dataResponse.status == true){
        ...
        $("#Unit").val(dataResponse.data[0].UnitCode).trigger('change');
        $('#Unit_Farmasi option:not(:selected)').prop('disabled', true);
        $('#Unit option:not(:selected)').prop('disabled', true);
    }else{
