...
async function createHeaderTrs() {
    try {
        ...
        const funcname = 'addReturJualHeader';
...
function updateUIdataGocreateHdr(dataResponse) {
    if(dataResponse.status == true){
        ...
        showDataDetil_DO($("#TransactionCodeReff").val());
        enableAll();
        $(".preloader").fadeOut();
    }else{
...
async function goSaveReturBeli() {
    try {
        ...
        const ctlrname = 'ReturJual';
        const funcname = 'addReturJualFinish';
...
function showDataDetil_DO(TransasctionCode) {
    ...
    $.ajax({
        type: "POST",
        data: "TransactionCode=" + TransasctionCode,
        url: base_url + url2,
...
function MyBack() {
    ...
    window.location = base_url + "/SIKBREC/public/ReturJual/list";
}
