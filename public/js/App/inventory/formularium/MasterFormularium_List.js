...
async function onloadForm() {
 ...
 function showdatatabel() {
     ...
     $('#example').DataTable({
         "ordering": true,
         "ajax": {
             "url": base_url + "/SIKBREC/public/MasterDataFormularium/getFormulariumAll",
             "dataSrc": "",
             "deferRender": true,
         },
         "columns": [
             ...
             },
             {
                 "render": function (data, type, row) {
                     var html = ""
                     var html = '<font size="1"> ' + row.Nama_Formularium + ' </font>  ';
                     return html
                 }
             }, 
             {
...
function showDataGroupShift(str) {
    const base_url = window.location.origin;
    var str = btoa(str);
    window.location = base_url + '/SIKBREC/public/MasterDataFormularium/' + str;
}

function goGroupShiftPages() {
    const base_url = window.location.origin;
    window.location = base_url + "/SIKBREC/public/MasterDataFormularium/";
}